...
public class Products {
	...
	List<String> productsHome = new ArrayList<String>();
	List<String> productsBasket = new ArrayList<String>();
	...
	public void getProductInfo() {
		List<WebElement> products = driver.findElements(dealOfDay);
		for (int i = 1; i < products.size(); i++) {
			productsHome.add(driver.findElements(dealOfDay).get(i).getAttribute("href"));
			System.out.println(productsHome.get(i - 1));
			if (i == 3)
				break;
		}
	}
	...
	public void checkHomeProducts() {
		...
		for (int i = 0; i < onBasket.size(); i++) {
			productsBasket.add(driver.findElements(productsOnBasket).get(i).getAttribute("href"));
			System.out.println(productsBasket.get(i));
			if (i == 1)
				break;
		}
	}

	public boolean verifyProducts() {//Sorulacak burası. Her türlü true dönüyor?
		Collections.sort(productsHome);
		Collections.sort(productsBasket);
		// System.out.println("HomePage "+productsHome);
		// System.out.println("Sepet "+productsBasket);
		
		boolean result = productsBasket.equals(productsHome);
		return result;
